...
# Cheap path: only runs full test on tracegraph-core (the only module with code in Phase 1).
set -e
...
echo "[pre-commit] running mvn -q -B -pl tracegraph-core test"
mvn -q -B -pl tracegraph-core test
