...
/**
 * In-memory {@link VectorStore} backed by a {@link ConcurrentHashMap}. Thread-safe.
 *
 * <p>Similarity is cosine similarity computed by brute-force scan. Suitable for development,
 * tests, and small collections (hundreds of vectors). For production workloads use an external
...
public final class InMemoryVectorStore implements VectorStore {
    ...
    public List<VectorMatch> query(String scope, float[] embedding, int topK) {
        ...
        List<VectorMatch> matches = new ArrayList<>(entries.size());
