...
const ApiRef = () => (
  <div className="max-w-[1100px] mx-auto px-6 lg:px-8 py-16 fade-up">
    ...
    <p className="mt-4 text-[16px] text-ink-600 dark:text-ink-400 max-w-2xl">
      ...
      All paths are relative to your application's context root. Responses are JSON unless noted.
    </p>

    <div className="mt-12 space-y-14">
      {window.API_GROUPS.map(grp => (
        <section key={grp.group}>
          <h2 className="font-display text-[28px] text-ink-950 dark:text-white tracking-tight font-medium mb-5">{grp.group}</h2>
          <div className="rounded-2xl border hairline overflow-hidden bg-white dark:bg-ink-950">
            {grp.endpoints.map((ep, i) => (
              <Endpoint key={i} ep={ep} first={i === 0} />
            ))}
          </div>
        </section>
      ))}
    ...
    <section className="mt-16">
      <h2 className="font-display text-[28px] text-ink-950 dark:text-white tracking-tight font-medium mb-4">Example: fork from a step</h2>
      <CodeBlock filename="curl" language="bash">{
`<span class="cm"># Fork run e9c4f1a2 from step 2 against the current graph</span>
$ curl -X POST <span class="st">"http://localhost:8080/tracegraph/traces/e9c4f1a2/replay?step=2"</span>
{
  <span class="st">"executionId"</span>: <span class="st">"f1d92cb0-..."</span>,
  <span class="st">"forkedFromExecutionId"</span>: <span class="st">"e9c4f1a2-..."</span>,
  <span class="st">"forkedFromStepIndex"</span>: 2,
...
}[m] || 'bg-ink-100 text-ink-700');

const Endpoint = ({ ep, first }) => (
  <div className={`grid grid-cols-[80px_1fr] gap-4 px-5 py-4 items-start ${!first ? 'border-t hairline' : ''}`}>
    <span className={`mono text-[11px] uppercase tracking-wider px-2 h-6 inline-flex items-center rounded ${methodColor(ep.m)}`}>{ep.m}</span>
