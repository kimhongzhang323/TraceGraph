...
### Added
- **MCP server** (`tracegraph-connectors`, `mcp/`): `GraphMcpServer` exposes compiled graphs as MCP tools over the stdio transport (newline-delimited JSON-RPC 2.0; `initialize`, `tools/list`, `tools/call`, `ping`). `McpServerTool.ofGraph(name, description, schema, graph, seedDecoder, resultEncoder)` wraps a `Graph<S>` as one tool — arguments decode to the seed state, the final state encodes to the tool's text result. Tool failures surface as `isError: true` results per MCP; protocol errors as JSON-RPC errors. Complements the existing `McpClient`.
- **Golden-trace eval gate in CI**: new `Eval gate` workflow runs `EvalCiGateIT` (`tracegraph-e2e`) on every PR and main push — a deterministic `RecordedLlmClient`-backed QA graph is re-run against its golden traces (`EvalRunner` with per-node assertions, step-for-step `TraceDiff`) and an exact-match `EvalSuite` with `minPassRate(1.0)`. Regressions in the executor, tracing, or replay machinery fail the build with no network dependency.
- **A2A agent discovery** (`tracegraph-a2a` + starter): `AgentCard(id, name, description, url, skills)` capability descriptor (shaped after the Google A2A agent card) with new `AgentBus.registerCard(...)`/`agentCards()` default methods (no-op/empty by default — existing buses keep compiling; `InMemoryAgentBus` stores cards, replace-by-id, listed ordered by id). The starter exposes `GET /a2a/agents` on the existing `A2AController`, behind the `tracegraph.a2a.api-key` filter.
- **`tracegraph-cli`** — new module: interactive terminal trace viewer for a running TraceGraph app (`java -jar tracegraph-cli.jar --url http://localhost:8080 [--api-key KEY]`). Browse recorded traces with arrow-key navigation, inspect per-step state (before/after with failure markers), and live-tail executions over `GET /tracegraph/stream` with color-coded START/ENTER/RETRY/EXIT/ERROR/COMPLETE events. Built on JLine; sends the API key via `X-Api-Key`.
- **RAG trace integration** (`tracegraph-rag`): retrieval is now visible in recorded traces — `Retriever.retrieve(ctx, scope, query, topK)` reports the query and ranked hits (`hits=N id:score …`) through `ctx.reportRawIO`, and `RagPipeline`'s `retrieve`/`rerank` nodes report chunk counts and rerank scores. `RagPipeline.Builder` gains `traceRecorder(...)` and `sensitiveDataLogging(boolean)` passthroughs; queries are prompt-shaped data, so capture respects the same opt-in as raw LLM I/O (default off).
- **Live execution stream** (`tracegraph-observability` `live/` + starter): `LiveTraceFeed` is a `TraceRecorder` that broadcasts `LiveTraceEvent`s (START/ENTER/RETRY/EXIT/ERROR/COMPLETE) to live subscribers; wire it via `Graph.Builder.traceRecorder(...)` and the starter auto-registers `GET /tracegraph/stream[?execution={id}]` as an `EventSource`-consumable SSE endpoint (the GET counterpart to the run-starting `POST /tracegraph/traces/stream`). Slow subscribers drop events past a 256-event buffer instead of blocking the executor — the durable record stays in the `TraceStore`. This implements the endpoint the UI docs page already advertised.
- **`CachingLlmClient`** (`tracegraph-connectors`, `cache/`): exact-match response cache over any `LlmClient`, keyed by `LlmRequest` equality (same semantics as `RecordedLlmClient` replay) with LRU eviction (`of(delegate[, maxSize])`, default 1024). Failures are never cached. Complements the embedding-based `SemanticLlmCache`.
...
### Fixed
- **`GraphMcpServer` tool ordering**: `tools/list` now returns tools in registration order on every JVM/OS. The server held them in a `Map.copyOf(...)` (unspecified iteration order), so the listing order was non-deterministic across platforms; now backed by an insertion-ordered `LinkedHashMap`.
- **`tracegraph-cli` cross-platform glyphs**: the trace viewer now detects the terminal's output charset at startup and falls back to an ASCII symbol set (`>`, `-`, `x`, `->`, …) when it isn't UTF-8, so the UI stays legible on legacy Windows consoles (cp1252/cp437) that render Unicode box-drawing/symbol glyphs as `?`. UTF-8 terminals are unchanged. JLine already handles ANSI enablement and key decoding across Windows/macOS/Linux.
