    ...
    api.traces
      ...
      .then((raw) => {
        if (raw && typeof raw === 'object') {
          setTrace(raw as ExecutionTrace)
          setActiveIdx(0)
        }
      })
      .catch(() => {})
...
function TopBar({ trace, traceList, activeIdx, onLoad }: {
  ...
  function handleFork() {
    api.traces
      ...
      .then((r) => alert(`Forked → new executionId: ${r.executionId}`))
  ...
  return (
    <div className="rounded-xl border hairline bg-white dark:bg-ink-950 px-4 py-2.5 flex items-center gap-3 mono text-[12px] text-ink-700 dark:text-ink-300 flex-wrap">
      ...
      <span className="text-ink-300 dark:text-ink-700">·</span>
      <span>graph <strong className="text-ink-950 dark:text-white">{trace.graph}</strong></span>
      <span className="text-ink-300 dark:text-ink-700">·</span>
      <span>{trace.startedAt}</span>
      <span className="text-ink-300 dark:text-ink-700">·</span>
      <span>{trace.duration}ms · {trace.steps.length} steps</span>
  ...
  return (
    <div className="mt-3 rounded-xl border hairline bg-white dark:bg-ink-950 px-4 py-3.5">
      <div className="flex items-center justify-between mono text-[11px] text-ink-500 mb-2.5">
        <span>timeline · {(trace.duration / 1000).toFixed(2)}s · virtual-thread executor</span>
...
function Logs({ trace }: { trace: ExecutionTrace }) {
